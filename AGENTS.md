...
discovers past agent-session transcripts tied to the repo or reads conversation exports the
user explicitly attaches, analyzes them, and proposes evidence-backed edits to `AGENTS.md` /
`CLAUDE.md` under a token budget.
...
- **Adapters drift and must stay fail-soft**: fix an adapter and its golden fixture together
  (`src/discovery/adapters/`), including explicitly selected OpenAI JSON and Nexus Markdown
  ChatGPT exports. Hermes ingests only `cli`/`acp` sessions (see its header).
